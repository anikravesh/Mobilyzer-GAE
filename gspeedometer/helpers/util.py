...
def ComputeMovingAverage(data_list):# (timestamp, value)
    min_value = -1
    sum_values = 0
    for t,v in data_list:
        sum_values=sum_values+float(v)
        if min_value==-1:
            min_value=float(v)
        elif min_value>float(v):
            min_value=float(v)
    avg=sum_values/len(data_list)
    sum_diff=0
    for t,v in sorted(data_list):
        sum_diff=sum_diff+((avg-float(v))*(avg-float(v)))
    stddev=math.sqrt(sum_diff/len(data_list))
    return (min_value,avg,stddev)

def Percentile(data, percentile):
    size = len(data)
    return sorted(data)[int(math.ceil((size * percentile) / 100)) - 1]

def SendGCMTask(devices_list,task):
    output_task = ConvertToDict(task, exclude_fields=['user', 'tag', 'filter'])
...
#     devices_list.append(regid)
    ...
    headers={'Content-Type': 'application/json', 'Authorization': "key="+"AIzaSyAyTVKfc6AwrPd6RKY18r4eNWJy3LHpz7Q"})
