...
class Archive(webapp.RequestHandler):
  ...
  def _Archive(self, **unused_args):
    ...
    if start_time:
      ...
      start = datetime.datetime(d0.year, d0.month, d0.day, d0.hour - 6, 0, 0, 0)
    if end_time:
...
#       gs_archive_name = '/gs/%s/%s.zip' % (bucket, archive_dir)
      gs_archive_name = '/%s/%s.zip' % (bucket, archive_dir)
      gs_archive = gcs.open(gs_archive_name,
                        'w',
                        content_type=config.ARCHIVE_CONTENT_TYPE,
                        options={'x-goog-acl': acl},
                        )
      logging.debug(gs_archive_name)
